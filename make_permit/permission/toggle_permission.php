    ...
    if(!isset($_GET['id']) || !isset($_GET['status']))  header('Location: ../../403.html');
        ...
        if($_GET['status']==='approve'){
            ...
            if($_SESSION['Type']==='T' && $_SESSION['APPROVE'] === true){
                $query="UPDATE `out_approve` SET `is_approved`='Y', `approve_username`='".$_SESSION['REQ_ID_INFO']."', `approved_at` = 'Y-m-d H:i:s' WHERE out_apply_id=".$_GET['id'].";";
            ...
            if($rows !== 0){
                ...
                if($_SESSION['Type']==='T' && $_SESSION['APPROVE'] === true) {
                    $query = "UPDATE `out_approve` SET `is_approved`='N', `approve_username`='', `approved_at`='' WHERE out_apply_id=" . $_GET['id'] . ";";
