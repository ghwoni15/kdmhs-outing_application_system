...
<?php
    ...
    if(!isset($_SESSION['TEMP_OP_NO'])) die("<script>location.href='./403.html';</script>\n");
?>
...
<div class="container">
        ...
        <fieldset>
            ...
            <?php
            /*UPDATE 수정 必*/
            if(isset($_SESSION['TEMP_MF_NO']) && isset($_POST['date']) && isset($_POST['startTime']) && isset($_POST['endTime']) && isset($_POST['reason'])){
                if($_SESSION['Type'] === 'T') $query = "UPDATE `outing_apply` SET `Date`='".$_POST['date']."',`startTime`='".$_POST['startTime']."',`endTime`='".$_POST['endTime']."',`Reason`='".$_POST['reason']."',`Approved`='T' WHERE `No`=".$_SESSION['TEMP_MF_NO'].";";
                else if($_SESSION['Type'] === 'S') $query = "UPDATE `outing_apply` SET `Date`='".$_POST['date']."',`startTime`='".$_POST['startTime']."',`endTime`='".$_POST['endTime']."',`Reason`='".$_POST['reason']."' WHERE `No`=".$_SESSION['TEMP_MF_NO'].";";
                ...
                $query = "SELECT DATE_FORMAT(o.begin_time, '%Y-%m-%d') AS Date, DATE_FORMAT(o.begin_time, '%H:%i') AS startTime, DATE_FORMAT(o.end_time, '%H:%i') AS endTime, o.note AS Reason FROM out_apply o WHERE o.id='".$_SESSION['TEMP_MF_NO']."';";
                $rs = mysqli_query($link, $query) or die("<script>location.href='./error.php?errno=1&errmsg=Wrong_Query';</script>\n");
                ...
                if($rn >= 1){
                    echo("<div class=\"control-group\">\n\t\t<h1>일련번호 ".$_SESSION['TEMP_MF_NO']."에 대한 외출정보입니다.</h1>\n\t</div>\n\t\t<br/><br/>\n");
