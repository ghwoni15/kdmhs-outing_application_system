...
//                        usernames[n++] = data[idx].username;
        ...
        function postInfo()
        {
            ...
            $.ajax({
                url:url,
                type:'POST',
                data:($('form').serialize()+("&username="+usernames[$('students').selectedIndex])),
                dataType:'JSON',
                success:function(data)
                {
                    alert("성공적으로 신청했다.");
...
<div class="container">
    ...
    <div id="right">
        <form method="POST" action="./make_permit/permission/send_permission.php?t_direct=true" onsubmit="postInfo;">
            <fieldset>
                <legend>생성</legend>
